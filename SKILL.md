...
※AIエージェントの残るVol.4/6/7/8は第3弾で差し替え予定。Vol.1は創刊号のため維持（Vol.5〜7のキャッチコピー欠落と同様、仕様策定前の状態として遡及修正の対象外）。
...
**用語解説の差し替え時に「来週予告」を確認するルール（Phase 49で確立）：**

既存号の用語解説を差し替える際は、必ず前号の「📌 来週予告」を読み、差し替え対象の用語（または関連テーマ）が名指しされていないか確認すること。
...
| 伴走支援（AI導入伴走モデル） | Vol.12 | Vol.24「FDE（フォワード・デプロイド・エンジニア）」と概念が近接するため温存。第3弾でVol.8の候補として検討予定 | Vol.8本文との結びつきを確認したうえで、FDEとの骨格の違い（FDEは個人の役割、伴走支援はサービス提供モデル）を明示できる場合 |
